...
from speed_to_lead.agents import build_opportunity_pipeline
...
async def test_complete_project_is_pursue() -> None:
    result = await build_opportunity_pipeline().run(_request())
...
async def test_incomplete_request_triggers_gap_fill() -> None:
    result = await build_opportunity_pipeline().run(
        _request(
